(function () {
    ...
    function buildAdminErrorModal() {
        const existing = document.getElementById("admin-ajax-error-modal");
        if (existing) {
            return existing;
        }
        const wrapper = document.createElement("div");
        wrapper.innerHTML = `
            <div class="modal fade" id="admin-ajax-error-modal" tabindex="-1" aria-hidden="true">
                <div class="modal-dialog modal-dialog-centered">
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            <p class="mb-0" id="admin-ajax-error-text">Не удалось выполнить операцию.</p>
                        </div>
    ...
    function showAdminAjaxError(message) {
        const text = (message || "").trim() || "Не удалось выполнить операцию.";
        const textNode = document.getElementById("admin-ajax-error-text");
        if (textNode) {
            textNode.textContent = text;
        }
        const modalEl = buildAdminErrorModal();
    ...
    async function extractAdminErrorMessage(response) {
    ...
    function initAdminAjaxFormInterception() {
        ...
        document.addEventListener("submit", async (event) => {
            ...
            event.preventDefault();
            if (form.dataset.pendingAjaxSubmit === "1") {
                return;
            }
            form.dataset.pendingAjaxSubmit = "1";
            ...
            try {
                ...
                if (!response.ok) {
                    showAdminAjaxError(await extractAdminErrorMessage(response));
                    return;
                }
                window.location.assign(response.url || window.location.href);
            } catch (error) {
                showAdminAjaxError(error instanceof Error ? error.message : "Не удалось выполнить операцию.");
            } finally {
                if (submitter instanceof HTMLButtonElement || submitter instanceof HTMLInputElement) {
                    submitter.disabled = false;
                }
                form.dataset.pendingAjaxSubmit = "0";
            }
        });
    }
