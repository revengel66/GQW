(function () {
    ...
    function renderDiagnostics(payload) {
        ...
        const html = `
            <div class="analytics-runtime-diag-cards">
                ${renderDiagCard("Исходные события", formatNullableInt(summary.rawRowsEstimate))}
                ...
                ${renderDiagCard("Trace в индексе логов", formatNullableInt(logIndex.traceLinks))}
            ...
            <p class="analytics-runtime-diag-description">
                Индекс логов помогает находить trace даже тогда, когда исходный файл уже перемещён в архив. Полные логи остаются в файлах, а в базе хранится только быстрый указатель и короткая диагностическая сводка.
            </p>
    ...
    function formatRuntimeGroupDescription(code, fallback) {
        const descriptions = {
            STORAGE: "Сроки хранения исходных событий и резервные настройки переноса старой истории.",
    ...
    function formatRuntimeSettingLabel(key, fallback) {
        const labels = {
            ...
            "analytics.raw.retention.days": "Срок хранения исходных событий",
            "analytics.raw.hot.days": "Период быстрых исходных данных",
            "analytics.raw.warm.days": "Период редко используемых исходных данных",
            ...
            "analytics.log-index.max-lines-per-trace": "Строк на один trace",
            "analytics.log-index.max-excerpts-per-trace": "Важных фрагментов на trace",
    ...
    function runtimeOverviewHelpPayload() {
        return {
            ...
            body: "В этом окне настраивается, сколько времени хранить исходные события, подготовленные данные для графиков и служебную информацию аналитики.",
            ...
            impact: "Свежие данные используются для быстрых графиков, а старая история может храниться дольше в более компактном виде. Подготовленные агрегаты помогают открывать большие периоды без тяжёлого чтения всех исходных событий.",
            safeValue: "Хорошее начальное правило: исходные события хранить около 90 дней, подготовленные почасовые и дневные данные - 2-3 года.",
    ...
    function actionHelpPayload(action) {
        const items = {
            ...
            lifecycle: {
                ...
                body: "Применяет правила хранения: удаляет устаревшие исходные данные, обновляет служебные отметки и готовит таблицы к дальнейшей работе.",
            ...
            logs: {
                ...
                body: "Ищет текущие и архивные файлы логов, строит быстрый индекс по trace и событиям. После этого вкладка «Логи (trace)» сможет подсказать, в каком архиве находится нужный лог.",
                when: "Когда trace не найден в текущем файле или после появления новых архивов.",
