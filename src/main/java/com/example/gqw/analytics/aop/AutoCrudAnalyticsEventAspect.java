...
import com.example.gqw.analytics.config.AnalyticsScanProperties;
...
public class AutoCrudAnalyticsEventAspect {
    ...
    private final AnalyticsScanProperties scanProperties;
    ...
    public AutoCrudAnalyticsEventAspect(
        ...
        AnalyticsScanProperties scanProperties
    ) {
    ...
    @Around("(@within(org.springframework.stereotype.Controller) || @within(org.springframework.web.bind.annotation.RestController)) && execution(public * *(..))")
    public Object aroundControllerCrud(ProceedingJoinPoint joinPoint) throws Throwable {
        if (!instrumentationPolicy.isEnabled()) {
            return joinPoint.proceed();
        }
        if (!isInScanPackage(joinPoint)) {
            return joinPoint.proceed();
        }
    ...
    private String resolveTraceId(HttpServletRequest request) {
        if (request != null) {
            Object attribute = request.getAttribute(AnalyticsTraceContext.TRACE_ID_REQUEST_ATTRIBUTE);
            if (attribute != null) {
    ...
    private boolean isInScanPackage(ProceedingJoinPoint joinPoint) {
        Object target = joinPoint.getTarget();
        Class<?> targetClass = target == null ? null : AopUtils.getTargetClass(target);
        if (targetClass == null && joinPoint.getSignature() instanceof MethodSignature methodSignature) {
            targetClass = methodSignature.getMethod().getDeclaringClass();
        }
        return scanProperties.matches(targetClass);
    }
